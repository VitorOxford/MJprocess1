                        ...
                        <v-row>
                          ...
                          <v-col cols="12" sm="6">
                            <v-autocomplete
                              ...
                              :custom-filter="(itemTitle, queryText, item) => itemTitle.toLowerCase().includes(queryText.toLowerCase()) || item.value.toString().includes(queryText)"
                            >
                               <template v-slot:item="{ props, item }">
...
const fetchInitialData = async () => {
    ...
    try {
        const [products, services, statuses, stamps, stockData] = await Promise.all([
...
const syncOrderWithGestaoClick = async () => {
    ...
    for (const item of orderItems.value) {
        ...
        if (product) {
            ...
            // **INÍCIO DA CORREÇÃO**
            // 1. Atualiza no Gestão Click
            await gestaoApi.atualizarEstoqueProduto(product.id, newStock);

            // 2. Atualiza no Supabase e checa alertas
            const stockItem = stockItems.value.find(s => s.gestao_click_id === product.id);
            if (stockItem) {
