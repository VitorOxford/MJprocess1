...
type OrderItem = {
  ...
  design_tag: 'Desenvolvimento' | 'Alteração' | 'Finalização' | 'Aprovado'; new_stamp_file?: File | null;
  has_insufficient_stock: boolean; status: 'design_pending' | 'pending_stock'; total_value_items?: number;
};
...
const DRAFTS_KEY = 'savedOrderDrafts';
...
const getSavedDrafts = (): Draft[] => {
  try {
    ...
    console.error("Erro ao ler rascunhos salvos:", e);
    return [];
  }
};

const persistDrafts = (allDrafts: Draft[]) => {
  localStorage.setItem(DRAFTS_KEY, JSON.stringify(allDrafts));
  drafts.value = allDrafts;
};

const saveDraftAs = () => {
  const clientName = orderHeader.customer_name || 'Cliente';
  const draftName = prompt('Dê um nome para este rascunho:', `Rascunho para ${clientName} - ${new Date().toLocaleDateString('pt-BR')}`);
  if (draftName) {
    isSavingDraft.value = true;
    const allDrafts = getSavedDrafts();
    const newDraft: Draft = { id: Date.now(), name: draftName, createdAt: new Date().toISOString(), data: orderState.value, };
    allDrafts.unshift(newDraft);
    persistDrafts(allDrafts);
    appStore.showSnackbar(`Rascunho "${draftName}" salvo com sucesso!`, 'success');
...
const loadDraft = (draftId: number) => {
  const allDrafts = getSavedDrafts();
  const draftToLoad = allDrafts.find(d => d.id === draftId);
  if (draftToLoad) {
    restoreState(draftToLoad.data);
    draftsModal.value = false;
    showDraftAlert.value = false;
    appStore.showSnackbar(`Rascunho "${draftToLoad.name}" carregado.`, 'info');
  }
};

const deleteDraft = (draftId: number) => {
  if (confirm('Tem certeza que deseja excluir este rascunho?')) {
    let allDrafts = getSavedDrafts();
...
const autoSave = () => {
  ...
  localStorage.setItem(AUTO_SAVE_KEY, JSON.stringify(orderState.value));
};
...
const restoreAutoSave = () => {
  try {
    ...
    if (savedState) {
      ...
      appStore.showSnackbar('Progresso restaurado.', 'info');
    }
    showDraftAlert.value = false;
  } catch (e) {
...
const clearAutoSave = (showSnack = false) => {
  ...
  showDraftAlert.value = false;
  if (showSnack) {
    appStore.showSnackbar('Rascunho descartado.', 'success');
    resetForm(false);
  }
};
...
watch(() => orderHeader.customer_id, (newId) => {
  if (newId) {
    ...
    if (client) {
      orderHeader.customer_name = client.nome;
    }
  } else {
...
const fetchNextOrderNumber = async () => { loadingNextOrderNumber.value = true; try { const { data, error } = await supabase.rpc('get_next_order_number'); if (error) throw error; nextOrderNumber.value = data; } catch (e) { console.error("Erro ao buscar próximo número do pedido:", e); nextOrderNumber.value = 0; } finally { loadingNextOrderNumber.value = false; } };
...
const resetForm = (clearDrafts = true) => {
...
const addWatermark = (doc: jsPDF) => { const totalPages = (doc as any).internal.getNumberOfPages(); const pageWidth = doc.internal.pageSize.getWidth(); const pageHeight = doc.internal.pageSize.getHeight(); for (let i = 1; i <= totalPages; i++) { doc.setPage(i); doc.setFontSize(72); doc.setTextColor(255, 192, 203); doc.saveGraphicsState(); doc.setGState(new (doc as any).GState({ opacity: 0.2 })); for (let y = 0; y < pageHeight; y += 100) { for (let x = -50; x < pageWidth; x += 150) { doc.text('RECIBO', x, y, { angle: -45, align: 'center' }); } } doc.restoreGraphicsState(); } };
...
const generateStandaloneReceiptPdf = async () => { if (!orderHeader.has_down_payment || !orderHeader.down_payment_proof_file) { appStore.showSnackbar('Nenhum comprovante de sinal encontrado para gerar o recibo.', 'error'); return; } isGeneratingPdf.value = true; try { const selectedClient = clientList.value.find(c => c.id === orderHeader.customer_id) || { nome: 'Cliente Desconhecido', cpf_cnpj: 'Não informado' }; const { data: orderData, error: orderError } = await supabase.from('orders').select('down_payment_proof_url').eq('id', createdOrderId.value).single(); if (orderError || !orderData?.down_payment_proof_url) { throw new Error('Não foi possível encontrar o link do comprovante de sinal.'); } const proofPublicUrl = orderData.down_payment_proof_url; const doc = new jsPDF(); const pageWidth = doc.internal.pageSize.width; const pageHeight = doc.internal.pageSize.height; const margin = 15; await addHeader(doc); doc.setFontSize(18).setFont('helvetica', 'bold').text(`Recibo de Sinal`, pageWidth / 2, 45, { align: 'center' }); const startY = 60; const receiptText = `O Estúdio de Estampa MJ (CNPJ: 20.631.721/0001-07) confirma o recebimento do comprovante de sinal referente ao Pedido #${String(createdOrderNumber.value).padStart(4, '0')}.`; const maxTextWidth = pageWidth * 0.67; const splitText = doc.splitTextToSize(receiptText, maxTextWidth); doc.setFontSize(11).setFont('helvetica', 'normal'); doc.text(splitText, margin, startY); const textHeight = doc.getTextDimensions(splitText).h; const qrSize = textHeight * 1.4; const qrX = margin + maxTextWidth + 5; const qrY = startY - 4; const qrCodeDataUrl = await QRCode.toDataURL(proofPublicUrl, { width: qrSize * 5 }); doc.addImage(qrCodeDataUrl, 'PNG', qrX, qrY, qrSize, qrSize); doc.setFontSize(7).setTextColor(100); doc.text('Aponte sua câmera aqui', qrX + qrSize / 2, qrY + qrSize + 4, { align: 'center' }); let nextElementY = startY + textHeight + 10; autoTable(doc, { startY: nextElementY, head: [['CLIENTE', 'CPF/CNPJ', 'DATA DE EMISSÃO']], body: [[selectedClient.nome, selectedClient.cpf_cnpj || 'Não informado', format(new Date(), 'dd/MM/yyyy', { locale: ptBR })]], theme: 'striped', }); const orderItemsForReceipt = orderItems.value.map(item => [ item.fabric_type || 'N/A', item.stamp_ref || 'N/A', `${item.quantity || 0}${item.unit_of_measure}`, item.unit_of_measure === 'kg' ? `~${item.quantity_meters?.toFixed(2) || 0}m` : '-' ]); doc.setFontSize(12).setFont('helvetica', 'bold').text('Itens do Pedido (Referência)', margin, (doc as any).lastAutoTable.finalY + 10); autoTable(doc, { startY: (doc as any).lastAutoTable.finalY + 15, head: [['Base', 'Estampa', 'Quantidade', 'Rendimento Aprox.']], body: orderItemsForReceipt, theme: 'grid', }); doc.addPage(); await addHeader(doc); let lastY = 60; doc.setFontSize(12).setFont('helvetica', 'bold').text('Comprovante Anexado', margin, lastY); const proofFile = orderHeader.down_payment_proof_file; if (proofFile && proofFile.type.startsWith('image/')) { const proofBase64 = await imageToBase64(proofFile); const imgProps = doc.getImageProperties(proofBase64); let imgWidth = imgProps.width; let imgHeight = imgProps.height; const contentWidth = pageWidth - margin * 2; const contentHeight = pageHeight - lastY - 20; const widthRatio = contentWidth / imgWidth; const heightRatio = contentHeight / imgHeight; const ratio = Math.min(widthRatio, heightRatio, 1); imgWidth *= ratio; imgHeight *= ratio; const x = (pageWidth - imgWidth) / 2; doc.addImage(proofBase64, 'PNG', x, lastY + 10, imgWidth, imgHeight); } addWatermark(doc); addFooter(doc); const pdfFileName = `Recibo_Sinal_Pedido_${String(createdOrderNumber.value).padStart(4, '0')}_${sanitizeName(selectedClient.nome)}.pdf`; doc.save(pdfFileName); appStore.showSnackbar('Recibo de sinal gerado e baixado com sucesso!', 'success'); } catch (error) { console.error("Erro ao gerar PDF do recibo:", error); appStore.showSnackbar(`Erro ao gerar PDF do recibo: ${error.message}`, 'error'); } finally { isGeneratingPdf.value = false; } };
