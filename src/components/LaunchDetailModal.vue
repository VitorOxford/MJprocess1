          ...
          <div v-for="item in order.order_items" :key="item.id" class="item-management-row">
            ...
            <div class="item-info">
              <v-text-field
                ...
                density="compact"
                hide-details
                class="font-weight-bold editable-field"
                @blur="updateItemReference(item)"
              ></v-text-field>
              <div class="text-caption">{{ item.fabric_type }} - {{ Number(item.quantity_meters).toLocaleString('pt-BR', { maximumFractionDigits: 2 }) }}m</div>
...
import ImageModal from './ImageModal.vue'; // Importa o novo componente
...
// --- Função para salvar a referência ---
const updateItemReference = async (item: any) => {
  try {
    const { error } = await supabase
      .from('order_items')
      .update({ stamp_ref: item.stamp_ref })
      .eq('id', item.id);
    if (error) throw error;
    // (Opcional) Adicionar um snackbar de sucesso
  } catch (err) {
    console.error("Erro ao atualizar a referência:", err);
